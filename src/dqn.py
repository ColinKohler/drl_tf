...
class DQN(object):
    ...
    def createNetwork(self, in_shape, out_shape, network_config)
        with tf.name_scope(self.name):
            ...
            queue_type = [tf.float32, tf.int32, tf.float32]
            queue_shape = [in_shape, [None], [None]]
            queue = tf.FIFOQueue(capacity=queue_size, dtypes=queue_types, shapes=queue_shape)
            ...
            for layer_config in network_config['layers']:
                layer_weights, layer_biases, layer = self.createLayer(prev_layer, layer_config)
            ...
            self.q_values_with_idxs = tf.gather_nd(q_values, q_value_idxs)

            action_onehot = tf.one_hot(self.batch_action, out_shape, 1.0, 0.0, dtype=tf.float32)
            q_values_acted = tf.reduce_sum(tf.multiply(q_values, action_onehot), axis=1)

            with tf.name_scope('loss'):
                err = tf.abs(batch_label - q_values_acted)
    ...
    def fcLayer(self, inp, out_shape, name, act=tf.nn.relu):
        with tf.variable_scope(name) as scope:
            ...
            activations = tf.nn.xy_plus_b(inp, weights, bias, name='activation')
